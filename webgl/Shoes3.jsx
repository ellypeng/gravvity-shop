...
export function Shoes3(props) {

  const [hovered, setHover] = useState(false)
  useCursor(hovered)
  const ref = useRef()
  
  const model_3_url = useStore((s) => s.model_3_url)

  const { scene } = useLoader(GLTFLoader, model_3_url)
  const copiedScene = useMemo(() => scene.clone(), [scene])
  const copiedScene1 = useMemo(() => scene.clone(), [scene])

  // console.log('model_3_url = ', model_3_url)
  // console.log('model_3_url = ', model_3_url == 'assets/models/shoes/shoes3.glb')
  ...
  return (
    <>
      {
        model_3_url === 'assets/models/shoes/shoes3.glb' ? 
        <>
          <primitive 
            {...props}       
            object={copiedScene1} 

...
import { FaRegSun, FaRegTimesCircle } from 'react-icons/fa'
import { toast, ToastContainer } from 'react-toastify'
import useStore from '../helpers/store'

export default function MyCart3D() {
  const popMyCart3D = useStore((s) => s.popMyCart3D)
  const popupModelIndex = useStore((s) => s.popupModelIndex)
  const shooping = useStore((s) => s.shooping)

  const objetos3D = useStore((s) => s.objetos3D)
  const [ipOpen1, setIpOpen1] = useState(false);
  const [ipOpen2, setIpOpen2] = useState(false);
  const [data, DataSet] = useState(objetos3D)
  ...
  const sumitChange = () => {
    let test3 = {
      ...
      name: data[popMyCart3D].nombre,
      category: data[popMyCart3D].categoria,
      img: data[popMyCart3D].imagen.src,
      price: data[popMyCart3D].precio,
  ...
  return (
    <div className="absolute"
      style={{
        display: popMyCart3D !== 0 ? 'flex' : 'none', zIndex: 2000000000,
        ...
        <div style={{ display: "flex", alignItems: "center", margin: "15px 0px 0px 0px", padding: '15px 15px 0px 15px' }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between" }}>
              <span style={{ fontWeight: 700, fontSize: '14px', fontFamily: 'Be Vietnam' }}>{data[popMyCart3D].categoria}</span>
              {/* <span style={{ fontWeight: "bold" }}>$123.32</span> */}
              <button onClick={() => { useStore.setState({ popMyCart3D: 0 }) }}><FaRegTimesCircle style={{ fontSize: '25px' }} /></button>
            </div>
            <h3 style={{ marginBottom: "5px", fontFamily: 'Poppins', fontSize: '25px', fontWeight: 600 }}>{data[popMyCart3D].nombre}</h3>
